...
//    float audioValue = ((texture2D(uAudioTexture, vec2(posX * uAudioZoom, posY * uAudioZoom)).r) * uAudioStrength);
    ...
    // On the edges of the shape use a medium value 
        ...
        audioValue = ((texture2D(uAudioTexture, vec2(0.5 * uAudioZoom, uv.y * uAudioZoom)).r) * uAudioStrength);
    }
...
//    float strength = cnoise(vec3(transformed + uTime));
//    transformed = vec3(transformed * strength * 10.0);
