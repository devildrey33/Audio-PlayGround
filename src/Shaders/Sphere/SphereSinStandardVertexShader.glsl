...
void main() {
    ...
    if (uv.x > 0.001 && uv.x < 0.999 && uv.y > 0.001 && uv.y < 0.999)  {
        audioValue = abs((texture2D(uAudioTexture, vec2(zoom + (uv.x / uAudioZoom), zoom + (uv.y / uAudioZoom))).b - 0.5) * 0.55) * uAudioStrength;
    }
    // On the edges of the shape use a medium value 
    else {
        audioValue = abs((texture2D(uAudioTexture, vec2(1.0, zoom + (uv.y / uAudioZoom))).b - 0.5) * 0.55) * uAudioStrength;
    }
